...
describe('MessageList', () => {
  ...
  it('shows the empty-state placeholder when there are no messages', () => {
    render(<MessageList roomId="room-1" sessionId="sess-1" handle="gustavo" soundEnabled={false} />)
  ...
  it('closes the EventSource connection on unmount', () => {
    const { unmount } = render(<MessageList roomId="room-1" sessionId="sess-1" handle="gustavo" soundEnabled={false} />)
    const es = MockEventSource.latest()
    unmount()
